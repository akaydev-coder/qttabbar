...
> Note: GitHub-hosted runners do not ship VS2010 (`v100`) C++ toolsets and some legacy installer/native prerequisites. The CI workflow therefore builds managed deliverables (plugin assemblies and shared managed libraries) as a compatibility baseline, and uploads the legacy project components (`QTTabBar`, `QTTabBarNative`, `QTHookLib`) as source artifacts.
